...
const ToastIcon: React.FC<{ type: ToastType }> = ({ type }) => {
  switch (type) {
    case 'success':
      return (
        <svg className="toast__icon" fill="currentColor" viewBox="0 0 20 20">
...
const ToastItem: React.FC<{ toast: Toast; onClose: () => void }> = ({ toast, onClose }) => {
  ...
  return (
    <div className={getToastClass()} role="alert">
      ...
      >
        <svg className="toast__close-icon" fill="currentColor" viewBox="0 0 20 20">
...
export const ToastContainer: React.FC = () => {
  ...
  return (
    ...
    >
      {toasts.map((toast) => (
        <div key={toast.id} className="toast-item-wrapper">
